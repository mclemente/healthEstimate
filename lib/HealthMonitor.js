...
export function outputStageChange(token) {
	try {
		...
		if (stage != game.healthEstimate.actorsCurrentHP[token.id].stage || state.dead != game.healthEstimate.actorsCurrentHP[token.id].dead) {
			...
			let desc = game.healthEstimate.descriptionToShow(game.healthEstimate.descriptions, stage, token, state, fraction);
			let oldDesc = game.healthEstimate.descriptionToShow(game.healthEstimate.descriptions, game.healthEstimate.actorsCurrentHP[token.id].stage, token, state, fraction);

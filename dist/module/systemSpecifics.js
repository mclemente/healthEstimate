...
function updateBreakConditions() {
	...
	breakOverlayRender = new Function(`token`,
		`return
			${prep("default")}
...
export function updateBreakSettings() {
	breakConditions["onlyGM"]   = game.settings.get("healthEstimate", "onlyGM")   ? `||!game.user.isGM`  : ``
	breakConditions["onlyNPCs"] = game.settings.get("healthEstimate", "onlyNPCs") ? `||token.actor.isPC` : ``
	updateBreakConditions()
}

export const prepareSystemSpecifics = new Promise((resolve, reject) => {
